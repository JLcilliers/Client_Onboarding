...
export async function getCompanyDetail(companyId: string): Promise<CompanyDetail | null> {
  ...
  const { data: questionnaire, error: questionnaireError } = await supabase
    .from("questionnaires")
    .select(
      "id,status,submitted_at,selected_services,updated_at",
    )
    .eq("company_id", companyId)
    .order("created_at", { ascending: false })
    .limit(1)
  ...
  if (questionnaire) {
    ...
    responses =
      ...
      >((acc, row) => {
        acc[row.section_key] = {
          updatedAt: row.updated_at ?? null,
          values: (row.responses as Record<string, unknown>) ?? {},
        };
        return acc;
      }, {}) ?? {};
  }
  ...
  const services =
    companyServices?.map((row) => ({
      key: row.services?.key ?? "",
      label: row.services?.label ?? row.services?.key ?? "",
      status: row.status,
    })) ?? [];
